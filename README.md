...
以下界面使用独立的本地演示数据生成，不包含真实业务信息。

### 管理端工作台

集中查看订单、库存预警、包装物和基础数据，并快速进入常用作业。

![ReplenOps 管理端工作台](docs/images/admin-dashboard.webp)

### 商品档案

通过统一的主数据工作区维护商品分类、规格、价格和启用状态。

![ReplenOps 商品档案](docs/images/goods-management.webp)

### 移动订货

门店人员可以按分类浏览商品、查看可用库存并完成订货。

<p align="center">
  <img src="docs/images/mobile-order.webp" width="390" alt="ReplenOps 移动订货界面">
</p>
...
访问令牌有效期为一小时，登录会话的绝对有效期为七天。受保护请求与 `/api/auth/session` 在访问令牌临近过期时自动续期；续期不延长七天上限。页面读取会话不修改 Cookie，续期由 Proxy 或显式刷新接口处理。

刷新令牌轮换通过 PostgreSQL 行锁协调多实例请求。同一代刷新令牌在轮换后的 30 秒内可重试并取得当前代令牌，重试不延长容错窗口；窗口外重放旧令牌会撤销会话。所有实例使用相同的 `JWT_SECRET`。注销、账号停用和权限变更后的失效检查不受容错窗口影响。

前台页面每四分钟检查会话，重新聚焦时也会检查。业务请求返回 `401` 后先确认会话状态，不自动重放写操作；会话服务暂时不可用不触发强制退出。

发布时先执行数据库迁移，再统一更新应用实例，避免混用不同的令牌轮换策略。
