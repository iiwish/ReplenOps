...
function validateStoreBindings(
  data: { roles: string[]; storeIds: string[] },
  context: z.RefinementCtx
) {
  const isStoreAdmin = data.roles.includes('STORE_ADMIN')

  if (isStoreAdmin && data.storeIds.length === 0) {
    context.addIssue({
      code: 'custom',
      path: ['storeIds'],
